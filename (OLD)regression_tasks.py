...
# from terratorch_registry import 
from registry import MODEL_FACTORY_REGISTRY #downloaded the relevant file since import doesn't work
from terratorch.tasks.loss_handler import LossHandler
from terratorch.tasks.optimizer_factory import optimizer_factory
# from terratorch.tasks.base_task import TerraTorchTask
from terratorch_basetask import TerraTorchTask
...
class RegressionTask(TerraTorchTask):  # changed from ClassificationTask
    ...
    def configure_losses(self) -> None:
        ...
        elif loss == "mse":  # added to handle regression
            ignore_value = -100 if ignore_index is None else ignore_index
            self.criterion = nn.MSELoss(ignore_index=ignore_value, weight=class_weights)  
        else:
    ...
    def training_step(self, batch: Any, batch_idx: int, dataloader_idx: int = 0) -> Tensor:
        ...
        other_keys = batch.keys() - {"image", "regr_label", "filename"} #ADD IN THE REGRESSION GROUND TRUTH
